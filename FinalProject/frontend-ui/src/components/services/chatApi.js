...
/* 
  대화내용 삭제
  변경 이유:
    - request()는 BASE_URL을 내부에서 결합하고, 성공 시 바로 본문을 반환.
    - 따라서 여기서 res.ok/res.json()을 다시 호출하면 런타임 오류/이중 처리 위험.
    - 또한 path는 항상 문자열("/chat/...")로 전달해야 하며, URL 객체/절대경로는 사용하지 않는다.
*/
export async function deleteMessage(messageId, { hard = false } = {}) {
  await request(`/chat/messages/${encodeURIComponent(messageId)}?hard=${hard ? 'true' : 'false'}`, {
    method: 'DELETE',
  });
  return true;
}

export async function deleteSession(sessionId, { hard = false } = {}) {
  await request(`/chat/sessions/${encodeURIComponent(sessionId)}?hard=${hard ? 'true' : 'false'}`, {
    method: 'DELETE',
  });
  return true;
}

export async function deleteOlderInSession(sessionId, { before, keep_last } = {}) {
  const params = new URLSearchParams();
  if (before) params.set('before', before);
  if (keep_last != null) params.set('keep_last', String(keep_last));
  const qs = params.toString();
  const path = `/chat/sessions/${encodeURIComponent(sessionId)}/messages${qs ? `?${qs}` : ''}`;
