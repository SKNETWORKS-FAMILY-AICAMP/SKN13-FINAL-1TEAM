// src/components/Modal/CreateorEditEventModal.jsx
...
function SelectMenu({
  ...
  const chevron = open ? <LuChevronUp className="h-4 w-4" /> : <LuChevronDown className="h-4 w-4" />;

  return (
    <div ref={rootRef} className={`relative ${disabled ? "opacity-50" : ""}`} aria-disabled={disabled}>
      <button
        id={id}
        type="button"
        onClick={() => {
        ...
        className={`h-11 w-full rounded-xl border border-gray-300 bg-white px-3 text-sm text-gray-700
                    flex items-center justify-between ${disabled ? "" : "hover:bg-gray-50"} ${className}
      ...
      {open && !disabled && (
        ...
        >
          <div className="max-h-64 w-full overflow-y-auto overscroll-contain simple-scroll">
            ...
            {!options?.length && (
              <div className="px-3 py-2 text-center text-sm text-gray-400">옵션 없음</div>
            )}
          </div>
        </div>
      )}
...
export default function CreateorEditEventModal({
  ...
  useEffect(() => {
    ...
    setDaysStart(Array.from({ length: maxS }, (_, i) => String(i + 1).padStart(2, "0")));
    if (Number(start.day) > maxS) setStart((s) => ({ ...s, day: String(maxS).padStart(2, "0") }));
  }, [start.year, start.month]);

  useEffect(() => {
    const maxE = getDaysInMonth(end.year, end.month);
    setDaysEnd(Array.from({ length: maxE }, (_, i) => String(i + 1).padStart(2, "0")));
    if (Number(end.day) > maxE) setEnd((s) => ({ ...s, day: String(maxE).padStart(2, "0") }));
  }, [end.year, end.month]);
  ...
  }, [start.year, start.month, start.day, start.time, end.year, end.month, end.day, end.time]); // eslint-disable-line

  const hasDateParts = (d) => String(d.year).trim() && String(d.month).trim() && String(d.day).trim();
      ...
      <div className="space-y-5">
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            <SelectMenu id="sy" value={start.year} onChange={(v) => setStart((s) => ({ ...s, year: v }))} options={years} className={numberBox} placeholder="YYYY" />
            <SelectMenu id="sm" value={start.month} onChange={(v) => setStart((s) => ({ ...s, month: v.padStart(2, "0") }))} options={months} className={monthBox} placeholder="MM" />
            <SelectMenu
              id="sd"
              value={start.day}
              onChange={(v) => setStart((s) => ({ ...s, day: v.padStart(2, "0") }))}
              options={Array.from({ length: getDaysInMonth(start.year, start.month) }, (_, i) => String(i + 1).padStart(2, "0"))}
              className={dayBox}
              placeholder="DD"
            />
            <SelectMenu
              id="st"
              value={start.time}
              onChange={(v) => setStart((s) => ({ ...s, time: v }))}
        ...
        <div>
          ...
          <div className="mt-2 flex items-center gap-3">
            <SelectMenu id="ey" value={end.year} onChange={(v) => setEnd((s) => ({ ...s, year: v }))} options={years} className={numberBox} placeholder="년도 선택" />
            <SelectMenu id="em" value={end.month} onChange={(v) => setEnd((s) => ({ ...s, month: v.padStart(2, "0") }))} options={months} className={monthBox} placeholder="월 선택" />
            <SelectMenu
              id="ed"
              value={end.day}
              onChange={(v) => setEnd((s) => ({ ...s, day: v.padStart(2, "0") }))}
              options={Array.from({ length: getDaysInMonth(end.year, end.month) }, (_, i) => String(i + 1).padStart(2, "0"))}
              className={dayBox}
              placeholder="일 선택"
            />
            <SelectMenu
            ...
            {TYPE_CHIPS.map((t) => {
              ...
              return (
                <button
                  ...
                  aria-pressed={active}
                >
                  <span className="inline-block h-2.5 w-2.5 rounded-full" style={{ backgroundColor: t.dot }} aria-hidden />
                  <span>{t.label}</span>
                </button>
              );
            })}
