/* 
  ...
  상하위 연결(데이터/이벤트 흐름):
    ...
    - props.onRemoveChat(sessionId): 삭제 성공 시 목록에서 제거(부모가 세션 상태 관리)
...
/* 
  ...
  인자(props):
    ...
    - onLogout: () => void
    - onRemoveChat: (sessionId: string) => void   // ✅ 추가: 세션 삭제 후 목록에서 제거
*/
export default React.memo(function Sidebar({
  ...
  // ★ 변경: onRemoveChat이 없을 때도 안전하게 동작하도록 no-op 제공
  const noop = useCallback(() => {}, []);
  const onRemoveChatSafe = onRemoveChat || noop;
          ...
          {safeSessions.length === 0 ? (
            ...
            safeSessions.map(({ chat, key, sid }) => (
              <ChatSummaryItem
                key={key}                          /* ⚙️ 안정 키 */
                title={chat.title}
                sessionId={sid}                    /* ✅ 삭제 API에 사용할 세션ID 전달 */
                onClick={makeSelectHandler(sid)}   /* ⚙️ 안전 선택 핸들러 */
                onRemoved={onRemoveChatSafe}       /* ✅ 삭제 성공 시 부모가 목록에서 제거 */
              />
            ))
          )}
        </div>
