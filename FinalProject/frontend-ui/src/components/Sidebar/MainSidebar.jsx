...
export default function MainSidebar({
    ...
    activeKey,
    onSelect = () => {},
    /** 🆕 추가: 로그아웃 버튼 클릭 시 외부에서 가로채서 모달을 띄울 수 있게 하는 콜백 */
    onLogoutClick, // 🆕
}) {
    ...
    return (
        ...
        >
            <div className={`flex items-center ${sidePad} pt-4 pb-2`}>
                ...
                {!collapsed && (
                    <h1 className="ml-3 font-semibold truncate">
                        {companyName}
                    </h1>
                )}
            ...
            {footer?.length ? (
                <div className="mt-auto px-3 pb-4 space-y-2 border-t border-gray-100 pt-3">
                    {footer.map((it) => {
                        ...
                        const Icon =
                            it.Icon ||
                            (isLogout ? BsBoxArrowRight : iconMap[it.key]);

                        // 🆕 변경: onLogoutClick이 주어지면 그것을 우선 사용하고, 없으면 기존 전역 로그아웃 호출
