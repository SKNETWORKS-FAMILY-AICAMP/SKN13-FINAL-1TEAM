...
def rule_based_intent_analysis(user_input: str) -> Dict[str, Any]:
    ...
    final_action_patterns = {
        ...
        'draft': [  # 새로운 초안 생성 패턴
            ...
            r'(문서|보고서).*?초안.*?(작성|만들어|생성)해?줘$',  # 문서 초안 작성 패턴
            r'미디어다양성.*?조사.*?용역.*?(작성|만들어|생성)해?줘$',  # 특정 문서 유형 생성
            r'공고문.*?미디어다양성.*?(작성|만들어|생성)해?줘$',
            r'미디어다양성.*?공고문.*?(작성|만들어|생성)해?줘$',  # 순서 바뀐 경우
            r'.*?(참조|기반).*?초안.*?(작성|만들어|생성)해?줘$',  # 참조 기반 초안 작성
            r'.*?(읽고|분석).*?(작성|만들어|생성)해?줘$',  # 분석해서 작성
            r'.*?보고서.*?(만들어|작성|생성)해?줘$'  # 보고서 생성 패턴
        ],
        'edit': [
