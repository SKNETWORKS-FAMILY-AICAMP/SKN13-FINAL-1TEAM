...
* **수집 방식 (해당 항목에 체크)**
  ...
  * [ ] API 호출
  * [ ] 사용자 입력
  * [x] **문서 파일 다운로드 (PDF, HWP 등)**
  * [ ] 기타: 내부 네트워크 공유 폴더 수동 수집

* **수집 도구 또는 스크립트 설명**:

  * 디렉토리 구조 기반 자동 탐색 및 메타데이터 수집
  * PDF 추출(`PyMuPDF`), HWP 추출(`PyHwp`) 후 chunking 및 저장
  * 파일명 기반 문서 제목, 버전(제정/개정), 날짜 메타 정보 추출 자동화

* **사용한 언어/라이브러리**:

  * `Python + os, re, PyMuPDF, python-hwp, tqdm, langchain`
...
* **중복 제거 기준**:
  ...
  * `title + version + date + chunk index` 기준으로 중복 규정 제거
